...
function calculateDirectBookingReadiness(scores) {
  ...
  const weighted = items.reduce((sum, item) => sum + (scores[item.key] * item.weight), 0);
...
function readinessDescription(score) {
  if (score == null) return 'The audit could not calculate a readiness score because one or more required category scores were unavailable during this run.';
...
function buildIssues({ mobile, desktop }) {
  ...
  if (mobilePerf == null) {
    issues.push(issue('Mobile performance could not be verified', 'The audit did not return a mobile Performance score for this URL during this run.', 'high'));
    ...
    issues.push(issue('Mobile performance needs improvement', `Mobile Performance score is ${mobilePerf}/100.`, 'medium'));
  }
  ...
  if (!issues.length) {
    issues.push(issue('Core website fundamentals look good', 'The audited category scores look strong for the checks included in this booth audit.', 'low'));
  }
...
function buildQuickFixes(mobile) {
  ...
  if (!opportunities.length) {
    return ['No major mobile improvement opportunities were returned during this run. Review the detailed category scores for next steps.'];
  }
...
app.post('/api/audit', async (req, res) => {
  ...
  try {
    ...
    let desktop = null;
    const errors = [];
    try { mobile = await runPageSpeed(url, 'mobile'); } catch (err) { errors.push(shortError(err)); }
    try { desktop = await runPageSpeed(url, 'desktop'); } catch (err) { errors.push(shortError(err)); }
    ...
    const report = {
      ...
      scoreSource: 'Direct Booking Readiness is calculated from the five visible category scores in this report.',
      ...
      readiness: {
        ...
        formula: 'Weighted from Mobile Performance, Desktop Performance, SEO, Best Practices, and Accessibility.'
      },
...
app.get('/api/leads.csv', (req, res) => {
  const leads = loadLeads();
  const columns = ['generatedAt', 'hotelName', 'prospectName', 'email', 'website', 'host', 'directBookingReadiness', 'mobilePerformance', 'desktopPerformance', 'seo', 'bestPractices', 'accessibility', 'topIssue', 'errors'];
...
app.listen(PORT, () => {
  console.log(`CHTA booth audit app running on http://localhost:${PORT}`);
});
